...
const ACShockHazardPage: React.FC = () => {
  const htmlContent = `
  <!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>AC Voltage Shock Hazard Simulator</title>
    ...
    <style>
        * {
        ...
        body {
            background: linear-gradient(135deg, #0a1a2a, #1a2e2e);
            color: #fff;
            display: flex;
        ...
        select {
            ...
            appearance: none;
            background-image: url("data:image/svg+xml;charset=UTF8,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 4 5'%3e%3cpath fill='%23aaeaff' d='M2 0L0 2h4zm0 5L0 3h4z'/%3e%3c/svg%3e");
        ...
        input[type="range"] {
            width: 100%; height: 8px; background: rgba(0, 200, 255, 0.2);
        ...
        input[type="range"]::-webkit-slider-thumb {
            ...
            box-shadow: 0 0 10px rgba(0, 212, 255, 0.8); transition: all 0.3s;
        }
        
        .radio-group { display: flex; flex-direction: column; gap: 8px; }
        .radio-option {
            padding: 8px 12px; background: rgba(0, 200, 255, 0.1);
            border: 1px solid rgba(0, 200, 255, 0.2); border-radius: 8px;
            cursor: pointer; transition: all 0.3s; user-select: none;
        }
        .radio-option:hover { background: rgba(0, 200, 255, 0.2); }
        .radio-option input { display: none; }
        .radio-option input:checked + span { color: #00d4ff; font-weight: bold; }
        .radio-option input:checked + span::before { content: '› '; }

        .start-button {
            width: 100%; padding: 12px; margin-top: auto;
            background: linear-gradient(45deg, #00d4ff, #00aaff); border: none;
            border-radius: 30px; color: #000; font-size: 1rem; font-weight: bold;
            cursor: pointer; transition: all 0.3s; text-transform: uppercase;
            box-shadow: 0 5px 15px rgba(0, 212, 255, 0.4);
        }
        
        .start-button:hover { transform: translateY(-3px); box-shadow: 0 8px 25px rgba(0, 212, 255, 0.6); }
        
        .visualization-area {
            position: relative; flex: 2; min-width: 300px;
            background: radial-gradient(ellipse at center, rgba(20, 30, 40, 0.8), rgba(0, 0, 0, 0.9));
            border-radius: 15px; display: flex; align-items: center; justify-content: center;
            overflow: hidden; border: 1px solid rgba(0, 200, 255, 0.2);
        }
        ...
        .current-unit { font-size: 1.2rem; color: #aaeaff; }
        ...
        .stat-item { background: rgba(0, 200, 255, 0.1); border: 1px solid rgba(0, 200, 255, 0.2); border-radius: 8px; padding: 8px; text-align: center; }
        .stat-label { font-size: 0.8rem; color: #aaeaff; margin-bottom: 4px; }
        .stat-value { font-weight: bold; color: #00d4ff; font-size: 1.1rem; }
        
        .calculation-breakdown {
            background: rgba(0, 0, 0, 0.3); border: 1px solid rgba(0, 200, 255, 0.2);
            border-radius: 8px; padding: 12px; margin-top: 15px; font-family: 'Courier New', Courier, monospace;
            font-size: 0.85rem;
        }
        .calculation-breakdown h3 { color: #00d4ff; margin-bottom: 10px; font-size: 1rem; font-family: 'Segoe UI', sans-serif; }
        .calc-line { display: flex; justify-content: space-between; margin-bottom: 5px; }
        .calc-label { color: #aaeaff; }
        .calc-value { color: #fff; font-weight: bold; }
        .calc-formula { color: #aaa; font-style: italic; font-size: 0.8rem; margin-top: 8px; border-top: 1px dashed rgba(0, 200, 255,0.2); padding-top: 8px;}
        #dynamic-guidance { color: #00ff88; margin-top: 10px; padding-top:10px; border-top: 1px solid rgba(0, 200, 255,0.2); font-size: 0.85rem; line-height: 1.4; }
        .disclaimer-box {
            padding: 12px; 
            margin-top: 15px;
...
<body>
    <header style="display: none;">
        <h1><i class="fas fa-wave-square"></i> AC Voltage Shock Hazard Simulator</h1>
        <div class="subtitle">Professional AC Electrical Safety Training System</div>
        <div class="standards">
    ...
    <div class="container">
            ...
            <div class="control-group">
                 ...
                 <select id="scenarioSelector">
                     ...
                     <optgroup label="Household">
                        <option value="household_us">Household (US, 120V)</option>
                        <option value="household_eu">Household (EU/Asia, 230V)</option>
                     </optgroup>
                     <optgroup label="Industrial">
                        <option value="industrial_277">Industrial Single-Phase (277V)</option>
                        <option value="industrial_480_unsafe">Industrial 3-Phase (480V - Unsafe)</option>
                        <option value="industrial_480_safe">Industrial 3-Phase (480V - Safe)</option>
                     </optgroup>
                     <optgroup label="Other">
                         <option value="generator">Portable Generator (240V)</option>
                     </optgroup>
                 </select>
            </div>
            <div class="control-group">
                <label class="control-label">AC Voltage (V)<span class="value-display" id="voltageValue">120 V</span></label>
                <input type="range" id="voltage" min="0" max="1000" value="120" step="1">
            </div>
             <div class="control-group">
                <label class="control-label">
                    <span>Frequency (Hz)</span>
                     <span class="info-icon">
                        ...
                        <span class="tooltip">AC frequency impacts shock severity. 50-60 Hz is the most dangerous range for the human heart. 60Hz is generally considered slightly more hazardous than 50Hz.</span>
                    </span>
                </label>
                <div class="radio-group" id="frequency">
                    <label class="radio-option"><input type="radio" name="freq" value="60" checked><span>60 Hz (Americas)</span></label>
                    <label class="radio-option"><input type="radio" name="freq" value="50"><span>50 Hz (Rest of World)</span></label>
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">
                    <span>Body Impedance (Ω)</span>
                    <span class="info-icon">
                        <i class="fas fa-info-circle"></i>
                        <span class="tooltip">For AC, we use 'impedance' which includes resistance and reactance. Like DC, it varies with skin moisture, contact area, etc. 1000Ω is a standard value for calculations.</span>
                    </span>
        ...
        <div class="panel">
            ...
            <div class="calculation-breakdown">
                ...
                <div class="calc-line"><span class="calc-label">Base Impedance:</span><span class="calc-value" id="calcBaseR">1,000 Ω</span></div>
                ...
                <hr style="border-color: rgba(0, 200, 255,0.2); margin: 5px 0;">
                <div class="calc-line"><span class="calc-label">Total Impedance:</span><span class="calc-value" id="calcTotalR">1,000 Ω</span></div>
                <div class="calc-formula">I = V / Z = <span id="calcOhm">--</span></div>
    ...
    <script>
        ...
        function setupEventListeners() {
            ...
            ['skinCondition', 'ppeStatus', 'environment', 'frequency'].forEach(id => {
        ...
        function handleScenarioChange(e) {
            ...
            const scenarios = {
                custom: null,
                household_us: { voltage: 120, resistance: 1500, skin: 'dry', ppe: 'none', env: 'dry', freq: 60 },
                household_eu: { voltage: 230, resistance: 1500, skin: 'dry', ppe: 'none', env: 'dry', freq: 50 },
                industrial_277: { voltage: 277, resistance: 1000, skin: 'dry', ppe: 'none', env: 'dry', freq: 60 },
                industrial_480_unsafe: { voltage: 480, resistance: 1000, skin: 'wet', ppe: 'none', env: 'dry', freq: 60 },
                industrial_480_safe: { voltage: 480, resistance: 1000, skin: 'dry', ppe: 'gloves', env: 'dry', freq: 60 },
                generator: { voltage: 240, resistance: 1000, skin: 'wet', ppe: 'none', env: 'wet', freq: 60 },
            };
            ...
            document.querySelector(\`input[name="freq"][value="\${s.freq}"]\`).checked = true;
        ...
        function toggleSimulation() {
            ...
            button.style.background = isSimulating ? 'linear-gradient(45deg, #ff4444, #ff8888)' : 'linear-gradient(45deg, #00d4ff, #00aaff)';
            if (isSimulating) {
                updateCalculations();
                startAnimation();
            } else { 
        ...
        function updateCalculations() {
            ...
            shockIntensity = Math.min(results.current / 100, 1);
            if (isSimulating) {
        ...
        function sendSafetyAlerts(results) {
            window.parent.postMessage({ type: 'SAFETY_ALERT', tool: 'ac-shock-hazard', key: 'FIBRILLATION_RISK', value: results.current }, '*');
            window.parent.postMessage({ type: 'SAFETY_ALERT', tool: 'ac-shock-hazard', key: 'CANNOT_LET_GO', value: results.current }, '*');
            window.parent.postMessage({ type: 'SAFETY_ALERT', tool: 'ac-shock-hazard', key: 'WET_CONDITIONS', value: results.isWet }, '*');
            window.parent.postMessage({ type: 'SAFETY_ALERT', tool: 'ac-shock-hazard', key: 'PPE_EFFECTIVE', value: results.hasPPE && results.current < 1.0 }, '*');
        }
        ...
        function updateAnalysisDisplays({ voltage, baseResistance, totalResistance, current, power, modifiers }) {
            ...
            if (current < 0.5) display.style.color = '#00ff00';
            else if (current < 5) display.style.color = '#ffff00';
            else if (current < 30) display.style.color = '#ffa500';
            ...
            if (current < 0.5) { indicator.className += ' severity-safe'; indicator.textContent = 'SAFE - Imperceptible'; }
            else if (current < 10) { indicator.className += ' severity-tingling'; indicator.textContent = 'Perception & Tingling'; }
            else if (current < 30) { indicator.className += ' severity-painful'; indicator.textContent = 'Painful Shock / Cannot Let Go'; }
            else if (current < 100) { indicator.className += ' severity-dangerous'; indicator.textContent = 'DANGEROUS - Fibrillation Possible'; }
            else { indicator.className += ' severity-fatal'; indicator.textContent = 'FATAL - Fibrillation Probable'; }
            ...
            let guidance = "";
            if(modifiers.ppe > 0) guidance = "Guidance: PPE is the most critical safety barrier, drastically increasing total impedance to safe levels.";
            else if (current > 30) guidance = "Guidance: Ventricular fibrillation is likely at this current level. This is a life-threatening emergency.";
            else if(modifiers.skin < 1.0) guidance = "Guidance: Wet skin dramatically lowers impedance, making even household voltages lethal.";
            else if(voltage > 50) guidance = "Guidance: Voltages above 50V AC are extremely hazardous due to the risk of ventricular fibrillation.";
        ...
        function drawHumanBody() {
            ctx.save();
            if (bodyGlow > 0) { ctx.shadowColor = \`rgba(100, 200, 255, \${bodyGlow})\`; ctx.shadowBlur = 20 * bodyGlow * scale; }
            ...
            ['leftArm', 'rightArm', 'leftLeg', 'rightLeg'].forEach(limb => {
                const vibration = shockIntensity * 4 * Math.sin(Date.now() * 0.1);
                ctx.beginPath(); ctx.moveTo(bodyModel[limb].x1 * scale, bodyModel[limb].y1 * scale); ctx.lineTo(bodyModel[limb].x2 * scale + vibration, bodyModel[limb].y2 * scale); ctx.stroke();
            });
            ...
            if (shockIntensity > 0.2) {
                ctx.strokeStyle = \`rgba(200, 255, 255, \${Math.min(1, (shockIntensity - 0.2) * 1.5)})\`;
            ...
            if (shockIntensity > 0.1) drawHeartbeat();
            ctx.restore();
        }
        ...
        function drawLightningBolt(x1, y1, x2, y2) {
            ctx.save();
            ctx.strokeStyle = \`rgba(170, 230, 255, \${0.4 + (shockIntensity * 0.6)})\`;
        ...
        function drawSparks(x, y, intensity) {
            for (let i = 0; i < Math.floor(15 * intensity); i++) {
                ...
                ctx.fillStyle = \`rgba(200, 240, 255, \${Math.random() * 0.9})\`;
                ctx.beginPath();
        ...
        function drawHeartbeat() {
            heartbeatAnimation += 0.1 + shockIntensity * 0.3;
            const isFibrillating = shockIntensity > 0.25;
            const beat = isFibrillating ? Math.random() * 0.9 + 0.1 : Math.abs(Math.sin(heartbeatAnimation)) * 0.8 + 0.2;
        ...
        function updateBodyEffects() {
            ...
            if (shockIntensity > 0.5) canvas.style.transform = \`translate(\${(Math.random() - 0.5) * 6 * shockIntensity}px, \${(Math.random() - 0.5) * 6 * shockIntensity}px)\`;
        ...
        function updateEnvironmentBackground() {
            ...
            bg.style.background = \`radial-gradient(ellipse at center, rgba(80, 150, 255, \${intensity}), rgba(20, 30, 70, 0.9))\`;
        }
...
export default ACShockHazardPage;